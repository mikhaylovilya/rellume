...
bool Lifter::Lift(const Instr &inst) {
  ...
  case farmdec::A64_ADRP: {
    // Scaling to page granularity is already handled in farmdec, but we need to
    // mask out the bottom 12 bits.
    auto masked_pc = irb.CreateAnd(AddrIPRel(), irb.getInt64(~(uint64_t)4095));
    SetGp(a64.rd, /*w32=*/false,
          irb.CreateAdd(masked_pc, irb.getInt64(a64.offset)));
    break;
  }
  ...
  case farmdec::A64_ORR_IMM:
    LiftBinOp(a64, w32, llvm::Instruction::Or, BinOpKind::IMM, set_flags);
    break;
  case farmdec::A64_EOR_IMM:
    LiftBinOp(a64, w32, llvm::Instruction::Xor, BinOpKind::IMM, set_flags);
    break;
  ...
  case farmdec::A64_MOVK: {
    ...
    val = irb.CreateAnd(
        val,
        irb.getIntN(bits, clrmask)); // clear 16 bits at point of insertion...
    val = irb.CreateOr(
        val, irb.getIntN(bits, shiftedimm)); // ...and fill in the immediate
  ...
  case farmdec::A64_SBFM:
    assert(
        false &&
        "SBFM should only appear as one of its aliases ASR_IMM, SBFIZ, SBFX");
    break;
  ...
  case farmdec::A64_SBFIZ: {
    ...
    auto sext_mask_value =
        irb.getIntN(bits, sext_mask); // truncates to 32-bit if required
    auto msb_set = irb.CreateICmpNE(irb.CreateAnd(field, sext_mask_value),
                                    irb.getIntN(bits, 0));
  ...
  case farmdec::A64_BFM:
    assert(false &&
           "BFM should only appear as one of its aliases BFC, BFI, BFXIL");
    break;
  case farmdec::A64_BFC: {
    uint64_t clrmask = ~(ones(a64.bfm.width) << a64.bfm.lsb);
    SetGp(a64.rd, w32,
          irb.CreateAnd(GetGp(a64.rd, w32), irb.getIntN(bits, clrmask)));
    break;
  }
  ...
  case farmdec::A64_UBFM:
    assert(false && "UBFM should only appear as one of its aliases LSL_IMM, "
                    "LSR_IMM, UBFIZ, UBFX");
    break;
  ...
  case farmdec::A64_UBFIZ:
    SetGp(a64.rd, w32,
          MoveField(GetGp(a64.rn, w32), w32, a64.bfm.lsb, a64.bfm.width));
    break;
  case farmdec::A64_UBFX:
    SetGp(a64.rd, w32,
          Extract(GetGp(a64.rn, w32), w32, a64.bfm.lsb, a64.bfm.width));
    break;
  case farmdec::A64_EXTEND:
    SetGp(a64.rd, w32,
          Extend(GetGp(a64.rn, w32), w32,
                 static_cast<farmdec::ExtendType>(a64.extend.type), 0));
    break;
  case farmdec::A64_EXTR: {
    ...
    auto mask = irb.getIntN(
        bits * 2, ones(bits)); // mask: 00000...(bits x 0):11111...(bits x 1)
    long_val = irb.CreateLShr(long_val, irb.getIntN(bits * 2, a64.imm));
    long_val = irb.CreateAnd(long_val, mask);

    SetGp(
        a64.rd, w32,
        irb.CreateTrunc(long_val, (w32) ? irb.getInt32Ty() : irb.getInt64Ty()));
    break;
  }
  ...
  case farmdec::A64_BCOND:
    SetIPCond(IsTrue(fad_get_cond(a64.flags)), inst.start() + a64.offset,
              inst.end());
    return true;
  case farmdec::A64_SVC:
    // SVC has an immediate, but cfg.syscall_implementation takes only a CPU
    // state pointer. This seems dire, but Linux and the BSDs and probably most
    // Unixes only ever use svc #0.
    ...
    return true;
    /*
        Intentionally unimplemented because they are either for debugging or use
       in the kernel while we focus on userspace lifting.
  ...
  case farmdec::A64_DMB:
    ...
    case 0xb:
    case 0x7:
    case 0x3: // SY, ISH, NSH, OSH (reads and writes)
      irb.CreateFence(llvm::AtomicOrdering::SequentiallyConsistent);
      break;
    case 0xe:
    case 0xa:
    case 0x6:
    case 0x2: // ST, ISHST, NSHST, OSHST (writes)
      irb.CreateFence(llvm::AtomicOrdering::Release);
      break;
    case 0xd:
    case 0x9:
    case 0x5:
    case 0x1: // LD, ISHLD, NSHLD, OSHLD (reads)
    ...
    break;
    /*
        Intentionally unimplemented since it can only affect PSTATE.D, .A, .I,
       .F, (DAIF), .SP, .SSBS, .PAN, .UAO, .DIT, all of which concern system
       decisions outside of LLVM/Rellume scope (e.g. exception masking, timing
       control).
  ...
  case farmdec::A64_MSR_REG: {
    ...
    case 0xda10: { // NZCV (bits 31-28)
      ...
      SetReg(ArchReg::SF,
             irb.CreateTrunc(irb.CreateLShr(nzcv, 31), irb.getInt1Ty()));
      SetReg(ArchReg::ZF,
             irb.CreateTrunc(irb.CreateLShr(nzcv, 30), irb.getInt1Ty()));
      SetReg(ArchReg::CF,
             irb.CreateTrunc(irb.CreateLShr(nzcv, 29), irb.getInt1Ty()));
      SetReg(ArchReg::OF,
             irb.CreateTrunc(irb.CreateLShr(nzcv, 28), irb.getInt1Ty()));
      break; // XXX
    }
    case 0xda20: // FPCR
  ...
  case farmdec::A64_SYS: {
    // DC ZVA -- zeroes a block of bytes, with the size stored in DCZID_EL0.
    // Both glibc and musl want 64-byte blocks, so do exactly that.
    if (a64.sys.op1 == 3 && a64.sys.op2 == 1 && a64.sys.crn == 7 &&
        a64.sys.crm == 4) {
      auto addr =
          GetGp(a64.rt, /*w32=*/false); // may point anywhere into the block
      auto start = irb.CreateAnd(
          addr, irb.getInt64(~0x3fuL)); // actual start address of block
  ...
  case farmdec::A64_MRS: {
    ...
    case 0xda10: { // NZCV (bits 31-28)
      ...
      nzcv = irb.CreateOr(
          nzcv,
          irb.CreateShl(irb.CreateZExt(GetFlag(ArchReg::SF), irb.getInt64Ty()),
                        31)); // nzcv |= n << 31
      nzcv = irb.CreateOr(
          nzcv,
          irb.CreateShl(irb.CreateZExt(GetFlag(ArchReg::ZF), irb.getInt64Ty()),
                        30)); // nzcv |= z << 30
      nzcv = irb.CreateOr(
          nzcv,
          irb.CreateShl(irb.CreateZExt(GetFlag(ArchReg::CF), irb.getInt64Ty()),
                        29)); // nzcv |= c << 29
      nzcv = irb.CreateOr(
          nzcv,
          irb.CreateShl(irb.CreateZExt(GetFlag(ArchReg::OF), irb.getInt64Ty()),
                        28)); // nzcv |= v << 28
      SetGp(a64.rt, /*w32=*/false, nzcv);
      break;
    }
    case 0xda20: // FPCR
      // Bits: AHP(26), DN(25), FZ(24), RMode(23:22), IDE(15), IXE(12), UFE(11),
      // OFE(10), DZE(9), IOE(8) All bits = 0 indicates IEEE 754 with round to
      // nearest and no exceptions.
      SetGp(a64.rt, /*w32=*/false, irb.getIntN(64, 0));
      break;
    case 0xda21: // FPSR
      // XXX We cannot check for "normal" FP errors, but we should store QC in
      // the CPU state and output it here. Bits: QC(27), IDC(7), IXC(4), UFC(3),
      // OFC(2), DZC(1), IOC(0).
      SetGp(a64.rt, /*w32=*/false, irb.getIntN(64, 0));
      break;
    case 0xd807: // DCZID_EL0
      SetGp(a64.rt, /*w32=*/false,
            irb.getIntN(
                64, 0x04)); // 4 → 64-byte block size wanted by musl and glibc
      break;
    case 0xc000: // MIDR_EL1
      // Bits: Implementer(31:24), Variant(23:20), Architecture(19:16),
      // PartNum(15:4), Revision(3:0). We set Architecture to 0b1111 to indicate
      // ARMv8, but we do not implement the feature registers ID_AA64* because
      // they are not commonly read in userspace (the compiler knows the
      // features of the target, so why would the generated code check them?).
  ...
  case farmdec::A64_BLR: {
    ...
    if (cfg.call_function) {
      ...
      // The external function call may manipulate the PC in non-obvious ways
      // (e.g. exceptions). See also the comment in the x86_64 LiftCall method.
  ...
  case farmdec::A64_CBNZ: {
    // CBZ: rt == 0; CBNZ: rt != 0
    auto pred = (a64.op == farmdec::A64_CBZ)
                    ? llvm::CmpInst::Predicate::ICMP_EQ
                    : llvm::CmpInst::Predicate::ICMP_NE;
    auto do_branch =
        irb.CreateICmp(pred, GetGp(a64.rt, w32), irb.getIntN(bits, 0));
  ...
  case farmdec::A64_TBNZ: {
    ...
    auto bit = irb.CreateAnd(GetGp(a64.rt, w32),
                             irb.getIntN(bits, mask)); // bit := rt & (1 << bit)

    // TBZ: bit == 0; TBNZ: bit != 0
    auto pred = (a64.op == farmdec::A64_TBZ)
                    ? llvm::CmpInst::Predicate::ICMP_EQ
                    : llvm::CmpInst::Predicate::ICMP_NE;
  ...
  case farmdec::A64_UDIV: {
    ...
    auto val = irb.CreateSelect(is_zero, irb.getIntN(bits, 0),
                                irb.CreateUDiv(lhs, safe_rhs));
  ...
  case farmdec::A64_SDIV: {
    ...
    auto is_min = irb.CreateAnd(irb.CreateICmpEQ(lhs, int_min),
                                irb.CreateICmpEQ(rhs, irb.getIntN(bits, -1)));
  ...
  case farmdec::A64_LSLV: {
    auto lhs = GetGp(a64.rn, w32);
    auto amount = irb.CreateAnd(GetGp(a64.rm, w32),
                                (w32) ? 0x1f : 0x3f); // lowest 5 or 6 bit
  ...
  case farmdec::A64_RORV: {
    ...
    auto fn = llvm::Intrinsic::getDeclaration(mod, llvm::Intrinsic::fshr,
                                              {lhs->getType()});
  ...
  case farmdec::A64_RBIT:
    SetGp(a64.rd, w32,
          irb.CreateUnaryIntrinsic(llvm::Intrinsic::bitreverse,
                                   GetGp(a64.rn, w32)));
    break;
  case farmdec::A64_REV16: { // swap bytes of every halfword independently
    auto vecty = llvm::VectorType::get(irb.getInt16Ty(), (w32) ? 2 : 4,
                                       false); // i32 → <2xi16>, i64 → <4xi16>
  ...
  case farmdec::A64_REV: // swap order of entire (double)word
    SetGp(a64.rd, w32,
          irb.CreateUnaryIntrinsic(llvm::Intrinsic::bswap, GetGp(a64.rn, w32)));
    break;
  case farmdec::A64_REV32: { // swap bytes of every word independently
    auto vecty = llvm::VectorType::get(irb.getInt32Ty(), (w32) ? 1 : 2,
                                       false); // i32 → <1xi32>, i64 → <2xi32>
    auto invec = irb.CreateBitCast(GetGp(a64.rn, w32), vecty);
    auto outvec = irb.CreateUnaryIntrinsic(llvm::Intrinsic::bswap, invec);
    SetGp(a64.rd, w32, irb.CreateBitCast(outvec, irb.getIntNTy(bits)));
    break;
  ...
  case farmdec::A64_CLZ: {
    ...
    SetGp(a64.rd, w32,
          irb.CreateBinaryIntrinsic(intr, val,
  ...
  case farmdec::A64_ORR_SHIFTED:
    LiftBinOp(a64, w32, llvm::Instruction::Or, BinOpKind::SHIFT, set_flags);
    break;
  ...
  case farmdec::A64_EOR_SHIFTED:
    LiftBinOp(a64, w32, llvm::Instruction::Xor, BinOpKind::SHIFT, set_flags);
    break;
  ...
  case farmdec::A64_CCMP_REG: {
    auto lhs = GetGp(a64.rn, w32);
    auto rhs = GetGp(a64.rm, w32);
    LiftCCmp(lhs, rhs, fad_get_cond(a64.flags), a64.ccmp.nzcv,
             (a64.op == farmdec::A64_CCMN_REG));
    break;
  }
  ...
  case farmdec::A64_CSEL: { // rd := (cond) ? rn : rm;
    ...
    SetGp(a64.rd, w32,
          irb.CreateSelect(IsTrue(fad_get_cond(a64.flags)), on_true, on_false));
    break;
  }
  ...
  case farmdec::A64_CSET: { // rd := (cond) ? 1 : 0;
    SetGp(a64.rd, w32,
          irb.CreateZExt(IsTrue(fad_get_cond(a64.flags)),
                         (w32) ? irb.getInt32Ty() : irb.getInt64Ty()));
    break;
  }
  ...
  case farmdec::A64_CINV: { // rd := (cond) ? ~rn : rn;
    auto val = GetGp(a64.rn, w32);
    SetGp(a64.rd, w32,
          irb.CreateSelect(IsTrue(fad_get_cond(a64.flags)), irb.CreateNot(val),
                           val));
    break;
  }
  case farmdec::A64_CSETM: { // rd := (cond) ? -1 : 0;
    SetGp(a64.rd, w32,
          irb.CreateSelect(IsTrue(fad_get_cond(a64.flags)),
                           irb.getIntN(bits, -1), irb.getIntN(bits, 0)));
    break;
  }
  ...
  case farmdec::A64_CNEG: { // rd := (cond) ? -rn : rn;
    auto val = GetGp(a64.rn, w32);
    SetGp(a64.rd, w32,
          irb.CreateSelect(IsTrue(fad_get_cond(a64.flags)), irb.CreateNeg(val),
                           val));
    break;
  }
  ...
  case farmdec::A64_SMULL:
    SetGp(a64.rd, /*w32=*/false,
          MulAddSub(GetGp(a64.ra, /*w32=*/false), llvm::Instruction::Add,
                    GetGp(a64.rn, /*w32=*/true), GetGp(a64.rm, /*w32=*/true),
                    llvm::Instruction::SExt));
    break;
  case farmdec::A64_SMSUBL:
  case farmdec::A64_SMNEGL:
    SetGp(a64.rd, /*w32=*/false,
          MulAddSub(GetGp(a64.ra, /*w32=*/false), llvm::Instruction::Sub,
                    GetGp(a64.rn, /*w32=*/true), GetGp(a64.rm, /*w32=*/true),
                    llvm::Instruction::SExt));
    break;
  case farmdec::A64_SMULH: {
    ...
    auto long_val = MulAddSub(irb.getIntN(64, 0), llvm::Instruction::Add, lhs,
                              rhs, llvm::Instruction::SExt);
  ...
  case farmdec::A64_UMULL:
    SetGp(a64.rd, /*w32=*/false,
          MulAddSub(GetGp(a64.ra, /*w32=*/false), llvm::Instruction::Add,
                    GetGp(a64.rn, true), GetGp(a64.rm, /*w32=*/true),
  ...
  case farmdec::A64_UMULH: {
    ...
    auto rhs = GetGp(a64.rm, /*w32=*/false);
    auto long_val = MulAddSub(irb.getIntN(64, 0), llvm::Instruction::Add, lhs,
                              rhs, llvm::Instruction::ZExt);
    auto high_half = irb.CreateLShr(long_val, irb.getIntN(128, 64));
    SetGp(a64.rd, /*w32=*/false, irb.CreateTrunc(high_half, irb.getInt64Ty()));
    break;
  }
  // Exclusive Loads and Stores always come in pairs and appear in a loop: first
  // the value is loaded and the address thus locked (LDXR), then the value is
  // compared and if necessary, a new value is stored to the previously locked
  // address (STXR). If the store fails (Rs == 1), the loop continues.
  //
  // Because we do not support true parallelism and because they always appear
  // in matching pairs, we can model LDXR/STXR as normal loads and stores, with
  // STXR always succeeding (Rs == 0).
  ...
  case farmdec::A64_PRFM: {
    ...
    auto mod = irb.GetInsertBlock()->getModule();
    auto fn =
        llvm::Intrinsic::getDeclaration(mod, llvm::Intrinsic::prefetch, tys);
    irb.CreateCall(fn, {addr, irb.getInt32(is_write), irb.getInt32(locality),
                        irb.getInt32(is_data)});
    break;
  }
  ...
  case farmdec::A64_FCVT_GPR: {
    ...
    if (a64.fcvt.fbits > 0) {
      auto scale = llvm::ConstantFP::get(TypeOf(fad_get_prec(a64.flags)),
                                         (double)(1LL << a64.fcvt.fbits));
    ...
    auto ival = (a64.fcvt.sgn) ? irb.CreateFPToSI(rounded, ity)
                               : irb.CreateFPToUI(rounded, ity);
  ...
  case farmdec::A64_CVTF: { // GPR(int|fixed) → Sca(fp)
    ...
    auto fp = (a64.fcvt.sgn) ? irb.CreateSIToFP(ival, TypeOf(prec))
                             : irb.CreateUIToFP(ival, TypeOf(prec));
  ...
  case farmdec::A64_FRINTX: {
    ...
    SetScalar(a64.rd,
              Round(GetScalar(a64.rn, prec),
                    static_cast<farmdec::FPRounding>(a64.frint.mode), exact));
    break;
  }
  ...
  case farmdec::A64_FABS:
    LiftIntrinsicFP(llvm::Intrinsic::fabs, fad_get_prec(a64.flags), a64.rd,
                    a64.rn);
    break;
  ...
  case farmdec::A64_FSQRT:
    LiftIntrinsicFP(llvm::Intrinsic::sqrt, fad_get_prec(a64.flags), a64.rd,
                    a64.rn);
    break;
  case farmdec::A64_FMUL:
    LiftBinOpFP(llvm::Instruction::FMul, fad_get_prec(a64.flags), a64.rd,
                a64.rn, a64.rm);
    break;
  case farmdec::A64_FDIV:
    LiftBinOpFP(llvm::Instruction::FDiv, fad_get_prec(a64.flags), a64.rd,
                a64.rn, a64.rm);
    break;
  case farmdec::A64_FADD:
    LiftBinOpFP(llvm::Instruction::FAdd, fad_get_prec(a64.flags), a64.rd,
                a64.rn, a64.rm);
    break;
  case farmdec::A64_FSUB:
    LiftBinOpFP(llvm::Instruction::FSub, fad_get_prec(a64.flags), a64.rd,
                a64.rn, a64.rm);
    break;
  case farmdec::A64_FMAX:
    LiftIntrinsicFP(llvm::Intrinsic::maxnum, fad_get_prec(a64.flags), a64.rd,
                    a64.rn, a64.rm);
    break;
  ...
  case farmdec::A64_FMIN:
    LiftIntrinsicFP(llvm::Intrinsic::minnum, fad_get_prec(a64.flags), a64.rd,
                    a64.rn, a64.rm);
    break;
  ...
  case farmdec::A64_FMADD: // (a*b) + c
    LiftFMA(fad_get_prec(a64.flags), a64.rd, a64.rn, a64.rm, a64.ra,
            /*neg_mult=*/false, /*sub=*/false);
    break;
  case farmdec::A64_FMSUB: // (-(a*b)) + c
    LiftFMA(fad_get_prec(a64.flags), a64.rd, a64.rn, a64.rm, a64.ra,
            /*neg_mult=*/true, /*sub=*/false);
    break;
  case farmdec::A64_FNMADD: // (-(a*b)) - c
    LiftFMA(fad_get_prec(a64.flags), a64.rd, a64.rn, a64.rm, a64.ra,
            /*neg_mult=*/true, /*sub=*/true);
    break;
  case farmdec::A64_FNMSUB: // (a*b) - c
    LiftFMA(fad_get_prec(a64.flags), a64.rd, a64.rn, a64.rm, a64.ra,
            /*neg_mult=*/false, /*sub=*/true);
    break;
  ...
  case farmdec::A64_FCCMPE: {
    ...
    auto lhs = GetScalar(a64.rn, prec);
    auto rhs = GetScalar(a64.rm, prec);
    LiftCCmp(lhs, rhs, fad_get_cond(a64.flags), a64.ccmp.nzcv, /*ccmn=*/false,
             /*fp=*/true);
    break;
  }
  case farmdec::A64_FCSEL: { // rd := (cond) ? rn : rm;
    ...
    SetScalar(a64.rd, irb.CreateSelect(IsTrue(fad_get_cond(a64.flags)), on_true,
                                       on_false));
    break;
  }
...
/// Get the value of an A64 general-purpose register. If w32 is true, the 32-bit
/// facet is used instead of the 64-bit one. Handles ZR (yields zero) and SP
/// (stack ptr).
...
/// Set the value of an A64 general-purpose register. If w32 is true, the 32-bit
/// facet is used instead of the 64-bit one. Handles ZR (discards writes) and SP
/// (stack ptr).
...
void Lifter::FlagCalcAdd(llvm::Value *res, llvm::Value *lhs, llvm::Value *rhs) {
  ...
  if (cfg.enableOverflowIntrinsics) {
    ...
    regfile->Set(ArchReg::OF, RegFile::Transform::AddOverflowFlag, res, lhs,
                 rhs);
  }
}
...
// and FSZ_Q (quad, Qr) return integers and may only be used with SIMD&FP
// load/stores. FSZ_S, FSZ_D return floating-point values if fp is true
// (default).
//
...
llvm::Value *Lifter::GetScalar(farmdec::Reg r, farmdec::FPSize fsz, bool fp) {
  ...
  case farmdec::FSZ_H:
    ...
    break; // XXX
  case farmdec::FSZ_S:
    fc = (fp) ? Facet::F32 : Facet::I32;
    break;
  case farmdec::FSZ_D:
    fc = (fp) ? Facet::F64 : Facet::I64;
    break;
  case farmdec::FSZ_Q:
    fc = Facet::I128;
    break;
...
// Shift or rotate the value v. No spurious instruction is generated if the
...
llvm::Value *Lifter::Shift(llvm::Value *v, farmdec::Shift sh, uint32_t amount) {
  ...
  switch (sh) {
  case farmdec::SH_LSL:
    return irb.CreateShl(v, (uint64_t)amount);
  case farmdec::SH_LSR:
    return irb.CreateLShr(v, (uint64_t)amount);
  case farmdec::SH_ASR:
    return irb.CreateAShr(v, (uint64_t)amount);
  case farmdec::SH_ROR:
    auto mod = irb.GetInsertBlock()->getModule();
    auto fn = llvm::Intrinsic::getDeclaration(mod, llvm::Intrinsic::fshr,
                                              {v->getType()});
    return irb.CreateCall(
        fn, {v, v, irb.getIntN(v->getType()->getIntegerBitWidth(), amount)});
  }
  return v; // no change
}
...
llvm::Value *Lifter::Extend(llvm::Value *v, bool w32, farmdec::ExtendType ext,
                            uint32_t lsl) {
  ...
  case farmdec::UXTB:
    ...
    break;
  case farmdec::UXTH:
    srcty = irb.getInt16Ty();
    sign_extend = false;
    break;
  case farmdec::UXTW:
    srcty = irb.getInt32Ty();
    sign_extend = false;
    break;
  case farmdec::UXTX:
  ...
  auto extended =
      (sign_extend) ? irb.CreateSExt(v, dstty) : irb.CreateZExt(v, dstty);
  return Shift(extended, farmdec::SH_LSL, lsl);
}

// Round the floating-point value v to an itegral floating-point value,
// according to the given rounding mode. If exact is true, FPR_CURRENT may raise
// an Inexact Exception. Whether an Inexact Exception is raised in the other
// cases is undefined behaviour
llvm::Value *Lifter::Round(llvm::Value *v, farmdec::FPRounding mode,
                           bool exact) {
  switch (mode) {
  ...
  case farmdec::FPR_TIE_EVEN:
    return irb.CreateUnaryIntrinsic(llvm::Intrinsic::roundeven, v);
  case farmdec::FPR_TIE_AWAY:
    return irb.CreateUnaryIntrinsic(llvm::Intrinsic::round, v);
  case farmdec::FPR_NEG_INF:
    return irb.CreateUnaryIntrinsic(llvm::Intrinsic::floor, v);
  case farmdec::FPR_ZERO:
    return irb.CreateUnaryIntrinsic(llvm::Intrinsic::trunc, v);
  case farmdec::FPR_POS_INF:
    return irb.CreateUnaryIntrinsic(llvm::Intrinsic::ceil, v);
  case farmdec::FPR_ODD:
    assert(false && "round to odd not supported");
  }
...
llvm::Type *Lifter::TypeOf(farmdec::FPSize fsz) {
  ...
  case farmdec::FSZ_B:
    return irb.getInt8Ty();
  case farmdec::FSZ_H:
    return irb.getInt16Ty(); // XXX
  case farmdec::FSZ_S:
    return irb.getFloatTy();
  case farmdec::FSZ_D:
    return irb.getDoubleTy();
  case farmdec::FSZ_Q:
    return irb.getIntNTy(128);
  }
...
llvm::Value *Lifter::IsTrue(farmdec::Cond cond) {
  ...
  switch (cond) {
  case farmdec::COND_EQ:
  case farmdec::COND_NE:
    res = GetFlag(ArchReg::ZF);
    break;
  case farmdec::COND_HS:
  case farmdec::COND_LO:
    res = GetFlag(ArchReg::CF);
    break;
  case farmdec::COND_MI:
  case farmdec::COND_PL:
    res = GetFlag(ArchReg::SF);
    break;
  case farmdec::COND_VS:
  case farmdec::COND_VC:
    res = GetFlag(ArchReg::OF);
    break;
  case farmdec::COND_HI:
  case farmdec::COND_LS:
    res = irb.CreateAnd(GetFlag(ArchReg::CF),
                        irb.CreateNot(GetFlag(ArchReg::ZF)));
    break;
  case farmdec::COND_GE:
  case farmdec::COND_LT:
    res = irb.CreateICmpEQ(GetFlag(ArchReg::SF), GetFlag(ArchReg::OF));
    break;
  case farmdec::COND_GT:
  case farmdec::COND_LE:
    res = irb.CreateAnd(
        irb.CreateNot(GetFlag(ArchReg::ZF)),
        irb.CreateICmpEQ(GetFlag(ArchReg::SF), GetFlag(ArchReg::OF)));
    break;
  case farmdec::COND_AL:
  case farmdec::COND_NV:
    return irb.getTrue(); // Both AL and NV yield true, so return directly
                          // without inverting.
  default:
...
llvm::AtomicOrdering Lifter::Ordering(farmdec::MemOrdering mo) {
  switch (mo) {
  case farmdec::MO_NONE:
    return llvm::AtomicOrdering::NotAtomic;
  case farmdec::MO_ACQUIRE:
    return llvm::AtomicOrdering::Acquire;
  case farmdec::MO_LO_ACQUIRE:
    return llvm::AtomicOrdering::Acquire; // Stronger than LOAcquire
  case farmdec::MO_ACQUIRE_PC:
    return llvm::AtomicOrdering::Acquire; // XXX is this correct? RCpc is weaker
                                          // than normal Acq, right?
  case farmdec::MO_RELEASE:
    return llvm::AtomicOrdering::Release;
  case farmdec::MO_LO_RELEASE:
    return llvm::AtomicOrdering::Release; // Stronger than LORelease
  }
  assert(false && "invalid memory ordering");
}

// Returns PC-relative address as i64, suitable for storing into PC again.
llvm::Value *Lifter::PCRel(uint64_t off) { return AddrIPRel(off); }
...
llvm::Value *Lifter::Addr(llvm::Type *elemty, farmdec::Inst inst) {
  ...
  // XXX But: might lead to clutter, and I'd like to keep the extensive
  // comments.
  ...
  case farmdec::AM_OFF_EXT:
    return Addr(elemty, inst.rn, inst.rm,
                static_cast<farmdec::ExtendType>(inst.extend.type),
                inst.extend.lsl);
  case farmdec::AM_PRE:
    SetGp(inst.rn, /*w32=*/false,
          irb.CreateAdd(GetGp(inst.rn, /*w32=*/false),
                        irb.getIntN(64, inst.imm))); // rn += imm
    return Addr(elemty, inst.rn);
...
llvm::Value *Lifter::Addr(llvm::Type *elemty, farmdec::Reg base) {
  return irb.CreatePointerCast(GetGp(base, false, /*ptr=*/true),
                               elemty->getPointerTo());
}
...
// AM_OFF_REG addressing mode: [base, Xoff {, LSL #imm}]. #imm is log2(size in
// bytes) or #0.
llvm::Value *Lifter::Addr(llvm::Type *elemty, farmdec::Reg base,
                          farmdec::Reg off, uint32_t lsl) {
  ...
  //     return elemptr + idx;
  //
  auto elemptr = irb.CreatePointerCast(GetGp(base, false, /*ptr=*/true),
                                       elemty->getPointerTo());
...
llvm::Value *Lifter::Addr(llvm::Type *elemty, farmdec::Reg base,
                          farmdec::Reg off, farmdec::ExtendType ext,
                          uint32_t lsl) {
  auto extended_off = GetGp(off, true);
  switch (ext) {
  case farmdec::UXTW:
    extended_off = irb.CreateZExt(extended_off, irb.getInt64Ty());
    break;
  case farmdec::SXTW:
    extended_off = irb.CreateSExt(extended_off, irb.getInt64Ty());
    break;
  default:
    assert(false &&
           "bad extend type for AM_OFF_EXT: only UXTW and SXTW are allowed");
  }
...
llvm::Value *Lifter::Extract(llvm::Value *v, bool w32, unsigned lsb,
                             unsigned width) {
  int bits = (w32) ? 32 : 64;
...
//
llvm::Value *Lifter::MoveField(llvm::Value *v, bool w32, unsigned lsb,
                               unsigned width) {
  int bits = (w32) ? 32 : 64;
...
// Given base, lhs, rhs : iN, calculate (base ± (lhs*rhs)) : i(N*2). This never
// overflows. The inputs are sign- or zero-extended before the operation.
llvm::Value *Lifter::MulAddSub(llvm::Value *base,
                               llvm::Instruction::BinaryOps addsub,
                               llvm::Value *lhs, llvm::Value *rhs,
                               llvm::Instruction::CastOps extend) {
...
// Lift an instruction Rd := Rn op X or, if invert_rhs is true, Rd := Rn op X,
// where X is an optionally inverted Shifted(Rm), Extended(Rm) or an Immediate.
// Most parameters are to avoid needless recalculations.
void Lifter::LiftBinOp(farmdec::Inst a64, bool w32,
                       llvm::Instruction::BinaryOps op, BinOpKind kind,
                       bool set_flags, bool invert_rhs) {
  auto lhs = GetGp(a64.rn, w32);
  llvm::Value *rhs = nullptr;
  switch (kind) {
  case BinOpKind::SHIFT:
    rhs = Shift(GetGp(a64.rm, w32), static_cast<farmdec::Shift>(a64.shift.type),
                a64.shift.amount);
    break;
  case BinOpKind::EXT:
    rhs = Extend(GetGp(a64.rm, w32), w32,
                 static_cast<farmdec::ExtendType>(a64.extend.type),
                 a64.extend.lsl);
    break;
  case BinOpKind::IMM:
    rhs = irb.getIntN((w32) ? 32 : 64, a64.imm);
    break;
  }
  if (invert_rhs) {
...
// If the condition holds, compare the values; otherwise set the flags according
// to nzcv.
void Lifter::LiftCCmp(llvm::Value *lhs, llvm::Value *rhs, farmdec::Cond cond,
                      uint8_t nzcv, bool ccmn, bool fp) {
  ...
  SetReg(ArchReg::SF, irb.CreateSelect(cond_holds, GetFlag(ArchReg::SF),
                                       irb.getInt1((nzcv & 8) != 0))); // N
  SetReg(ArchReg::ZF, irb.CreateSelect(cond_holds, GetFlag(ArchReg::ZF),
                                       irb.getInt1((nzcv & 4) != 0))); // Z
  SetReg(ArchReg::CF, irb.CreateSelect(cond_holds, GetFlag(ArchReg::CF),
                                       irb.getInt1((nzcv & 2) != 0))); // C
  SetReg(ArchReg::OF, irb.CreateSelect(cond_holds, GetFlag(ArchReg::OF),
                                       irb.getInt1((nzcv & 1) != 0))); // V
}

// Given a pointer ptr = *T, load a T value, extend it according to ext and put
// it in rt.
...
void Lifter::Load(farmdec::Reg rt, llvm::Type *srcty, llvm::Value *ptr,
                  farmdec::MemOrdering mo) {
  llvm::LoadInst *load = irb.CreateAlignedLoad(srcty, ptr, llvm::Align(1));
  if (mo != farmdec::MO_NONE) {
    load->setOrdering(Ordering(mo));
...
// Given a pointer ptr = *T, store the value val, which is truncated
// appropriately.
void Lifter::Store(llvm::Value *ptr, llvm::Value *val,
                   farmdec::MemOrdering mo) {
  llvm::StoreInst *store = irb.CreateStore(val, ptr);
  if (mo != farmdec::MO_NONE) {
    store->setOrdering(Ordering(mo));
    store->setAlignment(
        llvm::Align(val->getType()->getPrimitiveSizeInBits() / 8));
  } else {
...
void Lifter::LiftLoadStore(farmdec::Inst a64, bool w32, bool fp) {
  ...
  farmdec::ExtendType ext =
      fad_get_mem_extend(a64.flags);             // General load/stores
  ...
  case farmdec::A64_STXP: // TODO: exclusive access
    Store(irb.CreateConstGEP1_64(memty, ptr, 1),
          irb.CreateTruncOrBitCast(GetGp(a64.rt2, w32), memty), mo);
    /* fallthrough */
  case farmdec::A64_STR:
...
// Essentially, FCMP as defined by the ARM manual's pseudocode function
// FPCompare.
...
void Lifter::LiftBinOpFP(llvm::Instruction::BinaryOps op, farmdec::FPSize prec,
                         farmdec::Reg rd, farmdec::Reg rn, farmdec::Reg rm) {
  auto lhs = GetScalar(rn, prec);
  auto rhs = GetScalar(rm, prec);
  SetScalar(rd, irb.CreateBinOp(op, lhs, rhs));
}

void Lifter::LiftFMA(farmdec::FPSize prec, farmdec::Reg rd, farmdec::Reg rn,
                     farmdec::Reg rm, farmdec::Reg ra, bool neg_mult,
                     bool sub) {
...
void Lifter::LiftIntrinsicFP(llvm::Intrinsic::ID op, farmdec::FPSize prec,
                             farmdec::Reg rd, farmdec::Reg rn) {
